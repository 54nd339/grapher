...
- **Strictly avoid barrel files (`index.ts` re-exports).** They cause module graph explosions, break Turbopack dev server performance, and prevent efficient tree-shaking.
- Consumers ALWAYS import directly from the specific file where the module is defined.
  - Good: `import { Button } from "@/components/ui/button"`
  - Bad: `import { Button, Slider, Kbd } from "@/components/ui"`
- Delete `index.ts` files that exist solely to re-export other files. 
- Co-locate types with their relevant implementation, or import them directly from `types/specific-file.ts`.
