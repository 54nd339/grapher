...
const api = {
  ...
  findIntersections(
    ...
    const isImplicitA = latexA.includes("=");
    const isImplicitB = latexB.includes("=");
    ...
    const getVal = (x: number) => {
      ...
      if (isImplicitA && !isImplicitB) {
        const yB = safeEval(fnB, { ...scope, x });
        return safeEval(fnA, { ...scope, x, y: yB });
      }
      if (!isImplicitA && isImplicitB) {
        const yA = safeEval(fnA, { ...scope, x });
        return safeEval(fnB, { ...scope, x, y: yA });
      }
      // Both implicit is much harder (2D root finding)
      // For now, let's treat B as algebraic if it looks like y=
      if (latexB.includes("y=")) {
        const yB = safeEval(fnB, { ...scope, x });
        return safeEval(fnA, { ...scope, x, y: yB });
      }
      return NaN;
    ...
    let prevVal = getVal(xMin);

    for (let i = 1; i <= samples && points.length < limit; i++) {
      const x = xMin + i * dx;
      const val = getVal(x);

      if (!isNaN(prevVal) && !isNaN(val) && prevVal * val < 0) {
        ...
        let interY = NaN;
        if (!isImplicitB) {
          interY = safeEval(fnB, { ...scope, x: interX });
        } else if (!isImplicitA) {
          interY = safeEval(fnA, { ...scope, x: interX });
        } else if (latexB.includes("y=")) {
          interY = safeEval(fnB, { ...scope, x: interX });
        }
